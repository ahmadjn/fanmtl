...
class ChaptersScraper {
  ...
  async scrapeChapter(novel, novelId, chapter, totalChapters, chapterList, novelDetail, novelDetailPath) {
    try {
      ...
      // Update novel detail
      novelDetail.scraped_chapters = Math.max(novelDetail.scraped_chapters || 0, chapter);
      await fs.outputJson(novelDetailPath, novelDetail, { spaces: 2 });

      console.log(`   💾 Saved: Chapter ${chapter.toString().padStart(4, ' ')}/${totalChapters} - ${chapterTitle}`);
      ...
      console.log(`   ❌ Failed: Chapter ${chapter}/${totalChapters} - ${error.message}`);
      this.failedChapters++;
    }
  }
  ...
  async scrape() {
    ...
    for (const novel of novels) {
      try {
        ...
        // Scrape chapters with true concurrency
        for (let chapter = startChapter; chapter <= totalChapters;) {
          // Fill up to concurrentLimit
          while (this.activePromises.size < this.concurrentLimit && chapter <= totalChapters) {
            const promise = this.scrapeChapter(
              novel,
              novelId,
              chapter,
              totalChapters,
          ...
          // Update progress
          if (chapter % 5 === 0 || this.activePromises.size === 0) {
